...
from model import (
    RNN_ENCODER,
    CNN_ENCODER,
    CNN_ENCODER_RNN_DECODER,
    BERT_CNN_ENCODER_RNN_DECODER,
    BERT_RNN_ENCODER,
)
...
class condGANTrainer(object):
    ...
    def build_models(self):
        ...
        if cfg.GAN.B_DCGAN:
            ...
            if cfg.TREE.BRANCH_NUM > 2:
                netsD.append(D_NET256())
            # TODO: if cfg.TREE.BRANCH_NUM > 3:sebnt
            # print(netG)
    ...
    def train(self):
        ...
        if self.args.distributed:
            ...
            print("moved the generator")
            netG = netG.module
            for i in range(len(netsD)):
                netsD[i] = nn.parallel.DistributedDataParallel(
                    netsD[i],
                    device_ids=[self.args.local_rank],
                    output_device=self.args.local_rank,
                    broadcast_buffers=False,
        ...
        if cfg.CUDA:
            noise, fixed_noise = noise.to(device), fixed_noise.cuda()
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            if epoch % cfg.TRAIN.SNAPSHOT_INTERVAL == 0 and get_rank()==0:  # and epoch != 0:
